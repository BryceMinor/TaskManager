...
while (userInput !== `CLOSE`) {
    ...
    if (userInput === `NEW`){
        ...
        // Continues looping and prompting the user until something is entered (NOTE: If the user does NOT type anything and just clocks OK, then the value returned is an empty string)
        while (newTask === ``){
            newTask = prompt(`Please Enter the new task:`);
        }
    ...
    if (userInput === `REMOVE`){
        ...
        // Verifies the user entered a whole numer (AKA Not a decimal) that is one of the options in the prompt (AKA Within the number range of 1 to the length of the tasks array)
